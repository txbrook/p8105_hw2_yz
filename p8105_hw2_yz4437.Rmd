...
Dataset `pols` has dimension of the dataset is `r nrow(pols)` with `r ncol(pols)`, with year ranging from `r min(pols$year)` to `r max(pols$year)`. It contains the number of republican/democratic in governors/senators/representatives on the associated date.

Dataset `snp` has dimension of the dataset is `r nrow(snp)` with `r ncol(snp)`, with year ranging from `r min(snp$year)` to `r max(snp$year)`. It contains the closing values of the S&P stock index on the associated date.

Dataset `unemp` has dimension of the dataset is `r nrow(unemp)` with `r ncol(unemp)`, with year ranging from `r min(unemp$year)` to `r max(unemp$year)`. It contains percentage of unemployment in that associated year and month.

Dataset `result` has dimension of the dataset is `r nrow(result)` with `r ncol(result)`, with year ranging from `r min(result$year)` to `r max(result$year)`. It contains number of certain party in government, S&P stock index and unemployment rate that associated year and month in one dataset.
